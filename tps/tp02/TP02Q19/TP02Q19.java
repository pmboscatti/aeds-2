...
class Lista {
    ...
    public void ordenaPorSelecao() {
        for (int i = 0; i < k; i++) {
            ...
            for (int j = i + 1; j < contadorTamanho; j++) {
                if ((listaPersonagens[j].getNome()).compareTo(listaPersonagens[menor].getNome()) < 0) {
                    menor = j;
                }

                numeroComparacoes += 2; // Incrementa o número de comparações
            }

            numeroComparacoes++;

            swapPersonagem(menor, i);
        }
    ...
    public void criarLog(long tempoInicial) {
        ...
        Arq.openWrite("matricula_selecaoParcial.txt");
        Arq.println(
